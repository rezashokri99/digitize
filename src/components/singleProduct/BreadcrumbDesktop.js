...
const BreadcrumbDesktop = () => {
    ...
    return (
        <>
            <span className="text-orange-500">{productType}</span>
          ...
          <span className="text-orange-500">{productBrand}</span>
          <span className="text-slate-800 dark:text-stone-100">
            <svg
              className="w-2 h-4 font-bold"
              stroke="currentColor"
          ...
          <span className="text-gray-600 dark:text-stone-100 text-sm h-[21px]">{productPersianName}</span>
        </>
    );
}
