...
const SortSection = ({}) => {
    ...
    return (
        <div className="flex items-center px-4  text-gray-400 rounded-lg">
            ...
            <button onClick={() => activeSortBtnHandler(1)} className={`hover:bg-gray-50 py-4 px-2 relative ${activeSortBtn === 1 && "text-slate-800 font-bold"}`}>
                <span>محبوب ترین</span>
                {activeSortBtn === 1 && <span className="h-1.5 w-1.5 rounded-full bg-orange-600 absolute top-2.5 left-2"></span>}
            </button>
            <button onClick={() => activeSortBtnHandler(2)} className={`hover:bg-gray-50 py-4 px-2 relative ${activeSortBtn === 2 && "text-slate-800 font-bold"}`}>
                <span>ارزان ترین</span>
                {activeSortBtn === 2 && <span className="h-1.5 w-1.5 rounded-full bg-orange-600 absolute top-2.5 left-2"></span>}
            </button>
            <button onClick={() => activeSortBtnHandler(3)} className={`hover:bg-gray-50 py-4 px-2 relative ${activeSortBtn === 3 && "text-slate-800 font-bold"}`}>
                <span>گران ترین</span>
                {activeSortBtn === 3 && <span className="h-1.5 w-1.5 rounded-full bg-orange-600 absolute top-2.5 left-2"></span>}
            </button>
            <button onClick={() => activeSortBtnHandler(4)} className={`hover:bg-gray-50 py-4 px-2 relative ${activeSortBtn === 4 && "text-slate-800 font-bold"}`}>
